  ...
  try {
    ...
    if (format === 'csv') {
      ...
      return new Response(csvRows.join('\n'), {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="markets-feeds-export-${new Date().toISOString().split('T')[0]}.csv"`
        }
      });
    }
    ...
    return new Response(JSON.stringify(exportData, null, 2), {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="markets-feeds-export-${new Date().toISOString().split('T')[0]}.json"`
      }
    });
