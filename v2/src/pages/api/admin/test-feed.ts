...
async function testFeedUrl(url: string): Promise<FeedTestResult> {
  ...
  try {
    ...
    try {
      ...
      // Extract lastBuildDate or updated
...
function generateRecommendations(testResult: FeedTestResult): string[] {
  ...
  if (testResult.responseTime > 10000) {
